                ...
                <div class="row g-4 mb-5">
                    ...
                    <div class="col-lg-5">
                        <div class="card-paper p-4 position-sticky" style="top: 90px;">
                            ...
                            <!-- Candidate Schedule Availability -->
                            <?php if (!empty($schedule_summary)): ?>
                                <div class="d-flex justify-content-between align-items-center py-2 px-3 bg-cream rounded-3 border border-line mb-3 small">
                                    <div class="d-flex align-items-center gap-2 text-ink fw-semibold">
                                        <i class="bi bi-clock-history text-accent"></i>
                                        <span>Schedule Availability</span>
                                    </div>
                                    <div class="text-ink fw-medium">
                                        <?php if (($schedule_summary['slot_count'] ?? 0) > 0): ?>
                                            <?= (int)$schedule_summary['slot_count'] ?> shift slots (~<?= (int)$schedule_summary['hours'] ?> hrs/wk)
                                        <?php else: ?>
                                            <span class="text-danger">0 slots declared</span>
                                        <?php endif; ?>
                                    </div>
                                </div>
                            <?php endif; ?>
