...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        if ($action === 'save_settings') {
            ...
            $default_model = trim($_POST['default_model'] ?? 'nvidia/nemotron-3.5-lightning-30b-a3b');
...
<div class="site-wrapper d-flex flex-column min-vh-100 bg-light">
    ...
    <main class="flex-grow-1 py-4">
        <div class="container-paper">
            ...
            <?php if ($test_result): ?>
                <div class="card shadow-sm rounded-4 mb-4 border-<?= $test_result['success'] ? 'primary' : 'danger' ?>">
                    <div class="card-header bg-white py-2 d-flex justify-content-between align-items-center">
                        ...
                        <span class="badge bg-<?= $test_result['success'] ? 'success' : 'danger' ?>">
                            <?= $test_result['success'] ? 'HTTP 200 OK (' . $test_result['latency_ms'] . 'ms)' : 'Test Failed' ?>
                        </span>
                    </div>
                    <div class="card-body p-3">
                        <div class="small mb-1 text-muted">Model: <code><?= htmlspecialchars($test_result['model']) ?></code></div>
                        <div class="p-3 bg-light rounded-3 border">
                            <p class="mb-0 text-ink"><?= nl2br(htmlspecialchars($test_result['reply'])) ?></p>
                        </div>
                        <?php if (!empty($test_result['notice'])): ?>
                        ...
                        <div class="card-body p-4">
                            <form method="POST" action="ai-settings.php">
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="form-text small text-muted">
                                        Used when visitors select "Auto" or do not specify a specific model.
                                    </div>
                                </div>
                        ...
                        <div class="card-body p-4 d-flex flex-column justify-content-between">
                            ...
                            <div class="mt-4 p-3 bg-light rounded-3 border">
                                ...
                                <ul class="small text-secondary mb-0 ps-3">
                                    ...
                                    <li>Auto-fallback across secondary models on 429/503.</li>
